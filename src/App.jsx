...
function VideoEmbed({ url }) {
  ...
  if (m) return <iframe src={`https://www.youtube.com/embed/${m[1]}`} style={{ width: "100%", height: 220, borderRadius: 10, border: "none" }} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />;
  // Facebook
  if (url.includes("facebook.com") || url.includes("fb.watch")) return <iframe src={`https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=false`} style={{ width: "100%", height: 220, borderRadius: 10, border: "none" }} allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen />;
  // Instagram
  m = url.match(/instagram\.com\/(p|reel|tv)\/([a-zA-Z0-9_-]+)/);
  if (m) return <iframe src={`https://www.instagram.com/${m[1]}/${m[2]}/embed/`} style={{ width: "100%", height: 400, borderRadius: 10, border: "none" }} allowFullScreen />;
  // TikTok
  m = url.match(/tiktok\.com\/@[^/]+\/video\/(\d+)/);
  if (m) return <iframe src={`https://www.tiktok.com/embed/v2/${m[1]}`} style={{ width: "100%", height: 400, borderRadius: 10, border: "none" }} allowFullScreen />;
  // Fallback: just a link
  return <a href={url} target="_blank" rel="noreferrer" style={{ display: "block", padding: "12px 16px", background: "#f0f9ff", borderRadius: 10, color: "#0284c7", fontSize: 13, fontWeight: 600, textDecoration: "none", marginBottom: 6 }}>🎬 Voir la vidéo</a>;
}
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
    <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: 20, maxWidth: 520, width: "100%", maxHeight: "90vh", overflow: "auto" }}>
      ...
      <div style={{ padding: "20px 24px 24px" }}>
        ...
        {(p.videos || []).length > 0 && <div style={{ marginTop: 20 }}><div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>🎬 Vidéos</div><div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{p.videos.map((v, i) => <VideoEmbed key={i} url={v} />)}</div></div>}
        ...
        {!showBook ? <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={book} style={{ flex: 2, background: "linear-gradient(135deg,#FF6B00,#FF8534)", color: "white", border: "none", padding: "12px", borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: "pointer", opacity: nights > 0 && gName && gPhone ? 1 : 0.5 }}>Payer avec Mobile Money</button>
          </div>
...
export default function App() {
  ...
  const [settings, setSettings] = useState({ phone: "+225 07 00 00 00 00", email: "contact@stays.com", whatsapp: "+22507000000", address: "Abidjan, Côte d'Ivoire", tagline: "La plateforme N°1 de réservation d'hébergements en Côte d'Ivoire." });
  ...
  return <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", background: "#FAFAF8", minHeight: "100vh", color: "#1a1a2e" }}>
    ...
    {page === "home" && <div style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
      ...
      {curS.type === "brand" && <div style={{ position: "relative", zIndex: 3, textAlign: "center", padding: "0 24px", maxWidth: 750, opacity: heroLoaded ? 1 : 0, transition: "all 0.8s ease 0.3s" }}>
        ...
        <div style={{ display: "inline-flex", background: "rgba(255,255,255,0.97)", borderRadius: 16, padding: 6, gap: 6, boxShadow: "0 20px 60px rgba(0,0,0,0.3)", flexWrap: "wrap", justifyContent: "center" }}>
          <select value={selCity} onChange={e => setSelCity(e.target.value)} style={{ border: "none", outline: "none", padding: "14px 16px", fontSize: 14, borderRadius: 12, background: "#f8fafc", color: "#1a1a2e", fontFamily: "inherit", fontWeight: 600, cursor: "pointer", minWidth: 140 }}>{CITIES.map(c => <option key={c.code} value={c.code}>{c.name}</option>)}</select>
          <input placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)} style={{ border: "none", outline: "none", padding: "14px 18px", fontSize: 15, borderRadius: 12, background: "transparent", minWidth: 180, color: "#1a1a2e", fontFamily: "inherit" }} />
          <button onClick={goList} style={{ background: "linear-gradient(135deg,#FF6B00,#FF8534)", color: "white", border: "none", padding: "14px 28px", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}><SearchIcon /> Rechercher</button>
        </div>
      </div>}
    ...
    {page === "pricing" && <div style={{ maxWidth: 900, margin: "0 auto", padding: "100px 20px 60px", textAlign: "center" }}>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", justifyContent: "center" }}>
        {[{ n: "Basic", p: "5 000", c: "#64748b", pop: false, f: ["1 établissement", "3 photos", "Contact WhatsApp"] }, { n: "Premium", p: "10 000", c: "#FF6B00", pop: true, f: ["3 établissements", "10 photos", "Badge Sponsorisé", "Position prioritaire", "Statistiques"] }, { n: "Enterprise", p: "15 000", c: "#7C3AED", pop: false, f: ["Illimité", "Photos illimitées", "Page dédiée", "Support 24/7", "Rapport mensuel"] }].map((pl, i) => <div key={i} style={{ flex: "1 1 260px", maxWidth: 300, background: "white", borderRadius: 16, padding: "28px 24px", border: pl.pop ? `2px solid ${pl.c}` : "1px solid #f1f5f9", position: "relative" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 40, marginBottom: 32 }}>
          ...
          <div style={{ flex: "1 1 130px" }}>
            ...
            {[{ l: "Inscrire mon bien", p: "register" }, { l: "Tarifs", p: "pricing" }, { l: "FAQ", p: "faq" }, { l: "Contact", p: "contact" }].map(({ l, p }) => <div key={l} onClick={() => { setPage(p); window.scrollTo(0, 0); }} style={{ fontSize: 13, marginBottom: 8, cursor: "pointer" }}>{l}</div>)}
          </div>
